...
degSummary<-function(
  ...
  names(df)[grep(len, names(df))]<-"len"
  names(df)[grep(gc, names(df))]<-"gc"
  print(df$len)
  
  db <- bioFun(
    df, lfc ="lfc", stat="fdr", a1="Avg1", a2="Avg2", 
  ...
  dg<-data.frame(
    ...
    Length_Bias=c(
      cor(df$lfc, log(df$len,2), method="spearman"),
      cor(db$lfc, log(db$len,2), method="spearman")
    ),
    GC_Bias=c(
      cor(df$lfc, log(df$gc,2), method="spearman"),
      cor(db$lfc, log(db$gc,2), method="spearman")
    )
  )
  dg
}
